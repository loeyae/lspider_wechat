...
class WechatDB(Mongo, BaseWechatDB):
    ...
    def __init__(self, connector, table=None, **kwargs):
        super(WechatDB, self).__init__(connector, table = table, **kwargs)
        ...
        if not 'addWechat' in indexes:
            collection.create_index('addWechat', name='addWechat')
    ...
    def insert(self, obj = {}):
        ...
        _id = super(WechatDB, self).insert(setting=obj)
        return obj['uuid']

    def update(self, id, obj = {}):
        obj['utime'] = int(time.time())
        return super(WechatDB, self).update(setting=obj, where={'uuid': int(id)}, multi=False)

    def update_many(self, obj = {},where=None):
        if where=={} or where==None:
            return
        obj['utime'] = int(time.time())
        return super(WechatDB, self).update(setting=obj, where=where, multi=True)
    ...
    def delete(self, id, where = {}):
        ...
        return super(WechatDB, self).update(setting=obj, where=where, multi=False)
